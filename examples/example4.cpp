...
CIGARFILTER_PLUGIN(bool filter(CIGAR& cigar, std::string& samline) {
  ...
  auto replace_inplace = [](std::string& str, std::string& from,
                            std::string& to) -> bool {
  ...
  // The first step is to create a new list of groups that contains everything
  // but the padding
  ...
  std::string newCIGARstring =
      newCIGARsstring.str();  // Convert the streamstring as std::string
