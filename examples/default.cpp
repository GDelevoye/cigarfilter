...
bool filter(CIGAR &cigar,std::string &samline){
    if(!(cigar.error) && !(cigar.empty)){
         ...
         * See https://www.geeksforgeeks.org/problem-in-comparing-floating-point-numbers-and-how-to-compare-them-correctly/
        */

        if (percent_of_identity >= (1 - 1e-9)) { // This is the closest we have to "if(percent_identity == 1.0)"
            return true;
        } else {
            return false;
        }
