...
int main(int argc, char* argv[]) {
   ...
   * If you disable the synchronization, then C++ streams are allowed to have
   ...
   * Tied streams ensure that one stream is flushed automatically before each
   * I/O operation on the other stream. By default cin is tied to cout to ensure
   * a sensible user interaction. But, here, we don't want that because there is
   * no human user
   */
