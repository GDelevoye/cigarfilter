...
/*
 * returns a boost plugin object, that allows dependency injection from a
 * dynamically linked library See
 * https://www.boost.org/doc/libs/master/doc/html/boost_dll/tutorial.html
 */
boost::shared_ptr<cigarfilter_plugin> get_plugin(std::string name) {
  ...
  // Boost is supposed to be able to decorate the name of the library (see
  // dll::load_mode::append_decorations) In practice however, it just does not
  // work for me (bug ?), so here is a workaround
  auto suffixes = {".so", ".lib", ".dll", ".a", ".dylib", ""};
  std::string right_extension = "";
  for (auto extension : suffixes) {
    auto suffixed_path =
        plugin_dir / std::string(name + std::string(extension));
  ...
  // Better to beg forgivenesss than ask permission : let's try what we have
  // (nb: the last try is "")

  plugin = dll::import_symbol<cigarfilter_plugin>(
      plugin_dir / (name + right_extension),
      "plugin"  // name of the symbol to import
  );  // If this fails, boost throws an exception saying that the file is
      // missing

  /*
   * Important note about "import" vs "import_symbol" in the dll module :
   * A renaming has occured recently in boost to avoid collision with the std
   * C++20 "import" func See : https://github.com/robotology/blocktest/issues/47
