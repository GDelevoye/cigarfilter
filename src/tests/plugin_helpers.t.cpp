...
TEST(plugin_helpers, plugindir) { // Will fail if cigarfilter_config cannot be found in PATH
...
TEST(plugin_helpers, get_default_plugin) {
    auto success = true;
    try{
        auto plugin = cf::get_plugin("default"); // Throws a boost exception in case of failure
    }
